...
exports.handler = async function(event) {
  if (event.httpMethod === "GET") {
    return jsonResponse(200, { ok: true, hasApiKey: Boolean(process.env.GEMINI_API_KEY), model: DEFAULT_MODEL, version: "pdf-layout-v1" });
  }
  ...
  try {
    console.log("Gemini request start", { model: DEFAULT_MODEL, difficulty: difficultyKey, grade, subject: standard.subject });
    console.time("gemini-api");
    const response = await postJsonHttps(`${GEMINI_ENDPOINT}${encodeURIComponent(DEFAULT_MODEL)}:generateContent`, { "x-goog-api-key": apiKey }, payload);
    ...
    return jsonResponse(200, { assessment, model: DEFAULT_MODEL, scorePlan, grade, band, subject: standard.subject, code: standard.code });
  } catch (error) {
